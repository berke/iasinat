...
pub trait BinaryIo where Self:Sized {
    fn write_bin<W:Write>(&self,wr:W)->Result<()>;
    fn read_bin<R:Read>(rd:R)->Result<Self>;
}
...
macro_rules! binary_io_implem_array {
	...
	impl<const N:usize,T> $end for [T;N] where Self:Sized,T:$end+Copy {
	    fn write_bin<W:Write>(&self,mut wr:W)->Result<()>
	    {
		for x in self {
		    x.write_bin(&mut wr)?;
		}
	    ...
	    {
		let x0 = T::read_bin(&mut rd)?;
		let mut x = [x0;N];
		for i in 1..N {
		    x[i] = T::read_bin(&mut rd)?;
		}
		Ok(x)
	    }
	}
